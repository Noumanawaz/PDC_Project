...
int main(int argc, char** argv) {
    ...
    // Set number of OpenMP threads from command-line argument (default to 3 if not provided)
    int thread_count = (argc > 1) ? atoi(argv[1]) : 3;
    if (thread_count < 1) {
        if (process_id == 0) {
            cerr << "Error: Thread count must be at least 1" << endl;
        }
    ...
    // Open results file
    ofstream result_stream("Hybrid_output_rank_" + to_string(process_id) + ".txt");
    ...
    // Open timing file
    ofstream timing_stream("Hybrid_timing_rank_" + to_string(process_id) + ".txt");
